...
export async function POST(req) {
  try {
    ...
    // Send email notification — primary mechanism
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #f9f9f9; border-radius: 12px;">
          <div style="background: #FFE500; width: 40px; height: 40px; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin-bottom: 24px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              ...
              <td style="padding: 12px 16px; background: #fff; border: 1px solid #eee; border-top: none; font-size: 15px; color: #1A1A1A; font-weight: 600;">${email}</td>
            </tr>
            <tr>
              ...
              <td style="padding: 12px 16px; background: #fff; border: 1px solid #eee; border-top: none; font-size: 15px; color: #1A1A1A; font-weight: 600;">${businessType || 'Not specified'}</td>
            </tr>
          </table>
    ...
    try {
      ...
      console.error('[waitlist] DB save failed (email still sent):', dbErr.message)
    }
