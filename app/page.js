  ...
  if (status === 'success') {
    return (
      <div className="flex items-center gap-3 py-4">
        ...
        <div>
          <p className="font-bold text-sm" style={{ color: '#fff' }}>You&apos;re on the list.</p>
          <p className="text-xs mt-0.5" style={{ color: '#888' }}>We&apos;ll reach out before we launch. No spam.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
      <input
        ...
        style={{ background: '#1A1A1A', border: '1.5px solid #2A2A2A', color: '#fff', outline: 'none' }}
      />
      <select
        value={bizType}
        ...
        style={{
          ...
          color: bizType ? '#fff' : '#555', outline: 'none', minWidth: '160px',
        }}
      >
        <option value="">Business type</option>
        {AUDIENCE.map(a => <option key={a.label} value={a.label}>{a.label}</option>)}
      ...
      <button
        ...
        style={{ background: '#FFE500', color: '#0A0A0A', opacity: status === 'loading' ? 0.7 : 1 }}
      >
        {status === 'loading' ? 'Joining…' : <>Join the Waitlist <ArrowRight size={15} /></>}
      </button>
      {status === 'error' && (
        <p className="text-xs mt-1" style={{ color: '#FF4444' }}>Something went wrong. Try again.</p>
      )}
    </form>
  )
}
  ...
  return (
    <>
      <style>{`
        ...
        .nav-link { transition: color 0.15s ease; color: #888; }
        .nav-link:hover { color: #fff; }

        .problem-card { transition: transform 0.2s ease, border-color 0.2s ease; }
        .problem-card:hover { transform: translateY(-3px); border-color: #333 !important; }

        .stealth-card { transition: border-color 0.2s ease, background 0.2s ease; }
        .stealth-card:hover {
          border-color: #FFE500 !important;
          background: rgba(255,229,0,0.03) !important;
        }

        .audience-cell { transition: background 0.15s ease, color 0.15s ease; }
        .audience-cell:hover {
      ...
      <div className="beckon-land min-h-screen" style={{ background: '#0A0A0A', color: '#fff' }}>
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: '#FFE500', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontWeight: 800, fontSize: 14, color: '#0A0A0A' }}>B</span>
              </div>
              <span style={{ fontWeight: 800, fontSize: 18, color: '#fff', letterSpacing: '-0.3px' }}>Beckon</span>
            </Link>
        ...
        <section style={{ maxWidth: 1100, margin: '0 auto', padding: '90px 20px 80px' }}>
          <div className="fu fu-1" style={{ marginBottom: 28 }}>
            <span style={{
              ...
              fontSize: 12, fontWeight: 600, color: '#888', letterSpacing: '0.05em',
            }}>
          ...
          <h1 className="fu fu-2" style={{
            ...
            letterSpacing: '-2.5px', maxWidth: '14ch', marginBottom: 24,
          }}>
            The Uber for{' '}
          ...
          <p className="fu fu-3" style={{ fontSize: 18, lineHeight: 1.7, color: '#888', maxWidth: '52ch', marginBottom: 40 }}>
        ...
        <div style={{ borderTop: '1px solid #1A1A1A', borderBottom: '1px solid #1A1A1A', background: '#0F0F0F', padding: '18px 20px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 32, alignItems: 'center' }}>
            {['Built for local, not e-commerce', 'Zero marketing expertise required', 'Works with your existing booking tool', 'Launching soon'].map((item, i) => (
        ...
        <section style={{ maxWidth: 1100, margin: '0 auto', padding: '90px 20px' }}>
          <div style={{ marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 'clamp(2rem, 4vw, 2.8rem)', fontWeight: 800, letterSpacing: '-1.5px', maxWidth: '22ch', lineHeight: 1.1 }}>
              The tools you have were built for someone else.
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {PROBLEMS.map((p) => (
              <div key={p.tag} className="problem-card"
                style={{ background: '#0F0F0F', border: '1px solid #1A1A1A', borderRadius: 16, padding: 28 }}>
                <span style={{
                  display: 'inline-block', padding: '4px 10px', borderRadius: 6,
                  fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase',
                  background: p.accentBg, color: p.accent, marginBottom: 16,
                }}>
                  {p.tag}
                </span>
                <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 10, letterSpacing: '-0.4px', lineHeight: 1.3 }}>
                  {p.headline}
                </h3>
                <p style={{ fontSize: 14, lineHeight: 1.7, color: '#666' }}>{p.body}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ marginBottom: 56, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
              <div>
                ...
                <h2 style={{ fontSize: 'clamp(2rem, 4vw, 2.8rem)', fontWeight: 800, letterSpacing: '-1.5px', lineHeight: 1.1 }}>
                  Three things,{' '}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
              {STEALTH_FEATURES.map((f) => (
                <div key={f.number} className="stealth-card"
                  style={{ background: '#0A0A0A', border: '1px solid #1A1A1A', borderRadius: 16, padding: 28, position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 20, right: 20, width: 28, height: 28, borderRadius: 8, background: '#1A1A1A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Lock size={13} color="#444" />
                  </div>
                  <span style={{ fontSize: 40, fontWeight: 800, color: '#1E1E1E', display: 'block', marginBottom: 20, letterSpacing: '-2px' }}>
                    {f.number}
                  </span>
                  <p style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.35, letterSpacing: '-0.4px', marginBottom: 14, paddingRight: 24 }}>
                    {f.teaser}
                  </p>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 10px', borderRadius: 6, background: 'rgba(255,229,0,0.08)', fontSize: 11, fontWeight: 600, color: '#FFE500', letterSpacing: '0.04em' }}>
                    {f.hint}
                  </div>
                </div>
              ))}
        ...
        <section style={{ maxWidth: 1100, margin: '0 auto', padding: '90px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', border: '1px solid #1A1A1A', borderRadius: 16, overflow: 'hidden' }}>
            {AUDIENCE.map((a, i) => (
              <div key={a.label} className="audience-cell"
                style={{
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 24, height: 24, borderRadius: 6, background: '#FFE500', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontWeight: 800, fontSize: 11, color: '#0A0A0A' }}>B</span>
              </div>
              <span style={{ fontWeight: 800, fontSize: 14, color: '#fff' }}>Beckon</span>
            </div>
            <p style={{ fontSize: 12, color: '#444' }}>Built for local businesses, by people who care about them.</p>
